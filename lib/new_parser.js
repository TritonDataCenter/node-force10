...
function
f10_h_iface_desc(ftc, disable, s, iface)
{
	if (disable) {
		if (s.w.length !== 1) {
			return (new VE('malformed NO DESCRIPTION: %j', s));
		}
	...
	if (s.w.length < 2) {
		return (new VE('malformed DESCRIPTION: %j', s));
	}

	iface.description = s.w.slice(1).join(' ');
	return (null);
}
...
f10_h_lldp(ftc, disable, s)
{
	...
	for (var i = 0; i < s.c.length; i++) {
		var err = f10_st_apply(ftc, s.c[i], section_types, null);
		if (err) {
			return (new VE(err, 'failed to parse config'));
		}
	}
...
f10_h_lldp_advertise(ftc, disable, s)
{
	...
	if (MANAGEMENT_TLVS.indexOf(s.w[2]) === -1) {
		return (new VE('malformed ADVERTISE section: %j', s));
	}

	lib_common.array_set(ftc.ftc_model.protocols.lldp.management_tlv,
	    s.w[2], !disable);
	return (null);
}
...
f10_h_ether(ftc, disable, s)
{
	...
	var section_types = [
		{ n: 'description', h: f10_h_iface_desc },
	...
	for (var i = 0; i < s.c.length; i++) {
		var err = f10_st_apply(ftc, s.c[i], section_types, iface);
		if (err) {
			return (new VE(err, 'failed to parse config'));
		}
	}
...
f10_h_vlan(ftc, disable, s)
{
	...
	for (var i = 0; i < s.c.length; i++) {
		var err = f10_st_apply(ftc, s.c[i], section_types, vlan);
		if (err) {
			return (new VE(err, 'failed to parse config'));
		}
	}
