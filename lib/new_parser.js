...
f10_get_iface(ftc, type, port)
{
	...
	if (!iface) {
		...
		case 'hundredGigE':
			...
			iface.storm_control_packets_sec = null;
			break;
...
f10_section(ftc, s)
{
	...
	var section_types = [
		...
		{ n: 'logging', h: f10_h_basic },
		{ n: 'service', h: f10_h_basic },
		{ n: 'aaa', h: f10_h_basic },
		{ n: ['ip', 'access-list'],
		    c: true,
		    h: function wrapper(ftc, disable, s) {
			var first = s.w.shift();
			s.w[0] = first.concat('-', s.w[0]);
			return f10_h_basic(ftc, disable, s);
		    }},
		{ n: 'mac-address-table', h: f10_h_basic },
		{ n: 'arp', h: f10_h_basic },
		{ n: 'ntp', h: f10_h_basic },
		{ n: 'tacacs-server', h: f10_h_basic },
		{ n: 'clock', h: f10_h_basic },
		{ n: 'wred-profile', h: f10_h_basic },
		...
		{ n: [ 'interface', 'Port-channel' ], h: f10_h_port_channel,
		    c:true },
		...
		{ n: [ 'snmp-server', 'enable' ], h: f10_h_snmp_prop },
		{ n: [ 'snmp-server', 'host' ], h: f10_h_snmp_prop },
		 ...
		 * XXX TODO ignoring the children; oh well
		 */
		{ n: 'line', h: f10_h_line, c: true },
...
{
	/*
	 * snmp-server community COMMUNITY [ro|rw] [access-list-name]
	 */
	if ((s.w.length !== 4 && s.w.length !== 5) ||
	    (s.w[3] !== 'ro' && s.w[3] !== 'rw')) {
		return (new VE('invalid SNMP-SERVER COMMUNITY section: %j', s));
	}

	var access_list_name = null;
	if (s.w.length === 5) {
		access_list_name = s.w[6];
	}

	ftc.ftc_model.servers.snmp.communities[s.w[2]] =  {
		perm: s.w[3],
...
/*
 * XXX the fact that this even exists is not ideal
 */
function
f10_h_iface_ignore(ftc, disable, s, iface)
{
	return (null);
}
...
function
f10_h_iface_stormcontrol(ftc, disable, s, iface)
{
	if (disable) {
		iface.storm_control_packets_sec = null;
		return (null);
	...
	if (s.w.length !== 4 || s.w[1] !== 'broadcast' || s.w[3] !== 'in') {
		return (new VE('malformed STORMCONTROL: %j', s));
	}

	var packets_sec = mod_jsprim.parseInteger(s.w[2], { allowSign: false });
	if (packets_sec instanceof Error) {
		return (new VE('malformed STORMCONTROL: %j', s));
	}

	iface.storm_control_packets_sec = packets_sec;
	return (null);
}

function
f10_h_iface_portmode(ftc, disable, s, iface)
{
	if (s.w.length !== 2 || s.w[1] !== 'hybrid') {
		return (new VE('malformed PORTMODE: %j', s));
	}
...
function
f10_h_iface_ip_unreachables(ftc, disable, s, iface)
{
	if (s.w.length !== 2) {
		return (new VE('malformed IP UNREACHABLES: %j', s));
	}

	iface.unreachables = !disable;
	return (null);
}
...
f10_h_iface_stp(ftc, disable, s, iface)
{
	 ...
	 */
	if (mod_jsprim.deepEqual(s.w, [ 'spanning-tree', 'rstp',
	    'edge-port' ])) {
		iface.stp_rstp_edgeport = true;
		return (null);
	}
...
f10_h_ether(ftc, disable, s)
{
	...
	var section_types = [
		...
		{ n: 'portmode', h: f10_h_iface_portmode },
		...
		{ n: 'storm-control', h: f10_h_iface_stormcontrol, d: true },
		/* XXX TODO let's not ignore this */
		{ n: 'port-channel-protocol', h: f10_h_iface_ignore, c: true },
	];
...
f10_h_vlan(ftc, disable, s)
{
	...
	var section_types = [
		...
		{ n: 'shutdown', h: f10_h_iface_shutdown, d: true },
		{ n: [ 'ip', 'address' ], h: f10_h_iface_ip_address, d: true },
		{ n: [ 'ip', 'unreachables' ], h: f10_h_iface_ip_unreachables,
		    d: true },
...
function
f10_h_port_channel(ftc, disable, s)
{
	if (s.w.length !== 3) {
		return (new VE('malformed INTERFACE PORT-CHANNEL section: %j',
		    s));
	}

	var pchannel = f10_get_iface(ftc, s.w[1], s.w[2]);
	...
	return (f10_section_children(ftc, s, section_types, pchannel));
}
...
f10_h_username(ftc, disable, s)
{
	 ...
	 * OR
	 * username admin password 7 blahblahblah
	 */
	if ((s.w.length !== 7 || s.w[2] !== 'password' ||
	    s.w[5] !== 'privilege') &&
	    (s.w.length !== 5 || s.w[2] !== 'password')) {
